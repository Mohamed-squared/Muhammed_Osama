...
const Projects = () => {
  return (
    <FadeIn delay={0.4}> {/* Wrap section content */}
      <section id="projects" className="py-24 max-w-6xl mx-auto px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            <FadeIn key={index} delay={0.15 * index} direction="right"> {/* Wrap each project card */}
              {/* Added h-full for consistent card height if needed */}
              <div className="bg-slate-800/50 p-6 rounded-lg shadow-lg border border-slate-700 hover:-translate-y-2 transition-transform duration-300 flex flex-col h-full">
